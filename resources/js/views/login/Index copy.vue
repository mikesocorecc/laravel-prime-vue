  ...
  export default { 
    ...
    methods: { 
      ...
      login(values, { resetForm }) { 
         useJwt.login( { email: this.email, password: this.password })
            .then(response => {
              ...
              this.$router.replace({path : getHomeRouteForLoggedInUser(userData.role) } ).then(() => {
              
              //   console.log(`Bienvenidas ${userData.name || userData.email}` )
              //   console.log(`Has iniciado con éxito ${userData.role}. ¡Ahora puedes comenzar a explorar!`)
